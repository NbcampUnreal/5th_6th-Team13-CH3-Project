...
float UCombatComponent::ComputeImpactDamage(const TScriptInterface<IHitDamageable>& Attacker, const TScriptInterface<IHitDamageable>& Defender) const
{
	...
	const float ImpactFactor = FMath::Clamp(Attacker->GetCurrentSpeed() / MaxS, 0.f, 1.f); // 현재 속도를 최대속도 기준으로 0 ~ 1 까지 스케일링

	const float MassRatio = FMath::Max(Attacker->GetSizeScale(), 0.0001f) / FMath::Max(Defender->GetSizeScale(), 0.0001f); // 크기 계산

	const float MassTerm = FMath::Max(MassRatio - 1.f, 0.f); // 1보다 크면 공격자가 더 크다 1보다 작으면 공격자가 작다.

	// 방어자 최대 체력 * (충격 속도 + 크기 차이)
...
void UCombatComponent::EnsureMidAndSetScalar(UMeshComponent* Mesh, FName Param, float Value) const
{
	...
	{
		UMaterialInterface* Met = Mesh->GetMaterial(i);
		UMaterialInstanceDynamic* MID = Cast<UMaterialInstanceDynamic>(Met);
		if (!MID)
		{
